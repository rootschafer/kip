...
use kip_rsync::{
	test_fixture::LocalTestTempDir, test_utils::assert_directories_equal, Destination, ProgressTracker, Rsync,
	RsyncStats,
};
...
impl SshTestContainer {
	...
	async fn start() -> Result<Self, String> {
		...
		if !keygen.status.success() {
			return Err(format!("ssh-keygen failed: {}", String::from_utf8_lossy(&keygen.stderr)));
		}
		...
		for i in 0..60 {
			...
			let ssh_test = Command::new("ssh")
				.args(&[
					"-i",
					&key_path, // Use our generated key
					"-o",
			...
			if i == 59 {
				...
				return Err(
					"SSH container failed to authenticate after key setup (timeout after 30 seconds)".to_string(),
				);
			}
		}
...
async fn test_ssh_backup_basic() {
	run_with_timeout(async {
		...
		// Create test source
		let src = LocalTestTempDir::new("ssh_backup_src").expect("Failed to create source temp dir");
		...
		eprintln!(
			"SSH backup completed: {} bytes, {} files",
...
async fn test_ssh_backup_with_progress() {
	run_with_timeout(async {
		...
		let src = LocalTestTempDir::new("ssh_progress_src").expect("Failed to create source");
...
async fn test_ssh_backup_incremental() {
	run_with_timeout(async {
		...
		let src = LocalTestTempDir::new("ssh_incremental_src").expect("Failed to create source");
		...
		// Modify source
		std::fs::write(src.path().join("modified_file.txt"), "Modified content for incremental test")
		...
		eprintln!(
			"First backup: {} bytes, Second backup: {} bytes",
...
async fn test_ssh_backup_large_file() {
	run_with_timeout(async {
		...
		let src = LocalTestTempDir::empty("ssh_large_src").expect("Failed to create source");

		// Create a 5MB file
		let large_data: Vec<u8> = (0..255).cycle().take(5 * 1024 * 1024).collect();
		std::fs::write(src.path().join("large_file.bin"), &large_data).expect("Failed to create large file");
