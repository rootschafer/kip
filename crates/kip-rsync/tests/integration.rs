    ...
    run_with_timeout(async {
        ...
        if let Ok(stats) = result {
            ...
            if stats.bytes_transferred > 0 {
                assert_directories_equal(src.path(), dst.path())
                    .expect("Directories not equal");
            }
        }
    })
    .await;
...
async fn test_backup_preserves_empty_dirs() {
    run_with_timeout(async {
        ...
        assert!(
            dst.path().join("empty_dir").exists(),
            "Empty directory should be preserved"
        );
        assert!(
            dst.path().join("empty_dir").is_dir(),
    ...
    run_with_timeout(async {
        ...
        #[cfg(unix)]
        {
            std::os::unix::fs::symlink(
                src.path().join("file1.txt"),
                src.path().join("test_symlink.txt")
            ).ok();
        }
        ...
        #[cfg(unix)]
        {
            ...
            if symlink_path.exists() {
                assert!(
                    symlink_path.symlink_metadata().unwrap().file_type().is_symlink(),
...
async fn test_backup_large_file() {
    run_with_timeout(async {
        ...
        // Verify size matches
        let src_size = std::fs::metadata(src.path().join("large_file.bin")).unwrap().len();
        let dst_size = std::fs::metadata(dst.path().join("large_file.bin")).unwrap().len();
