...
impl Rclone {
	...
	pub async fn check_installed() -> Result<bool> {
		let output = Command::new("rclone").arg("--version").output().await;
	...
	async fn run_rclone_command(&self, command: &str, source: &Path, dest: &str) -> Result<CloudStats> {
		...
		let source_str = source
			.to_str()
			.ok_or_else(|| CloudError::InvalidPath("Source path contains invalid UTF-8".to_string()))?;
		...
		if !status.success() {
			...
			if error_msg.contains("authentication failed") || error_msg.contains("oauth") {
				return Err(CloudError::AuthenticationFailed(dest.to_string(), stderr_output));
			}
