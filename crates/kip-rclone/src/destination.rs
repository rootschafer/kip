...
impl WebDAVConfig {
	pub fn new(remote_name: impl Into<String>, folder_path: impl Into<String>, url: impl Into<String>) -> Self {
		Self {
			remote_name: remote_name.into(),
			folder_path: folder_path.into(),
			url: url.into(),
...
impl CloudDestination {
	...
	pub fn full_path(&self) -> String {
		match self {
			...
			Self::Generic { remote_name, path, .. } => format!("{}:{}", remote_name, path),
		}
	}
	...
	pub fn webdav(remote_name: impl Into<String>, path: impl Into<String>, url: impl Into<String>) -> Self {
	...
	pub fn generic(remote_name: impl Into<String>, path: impl Into<String>) -> Self {
		Self::Generic { remote_name: remote_name.into(), path: path.into() }
	}
}
...
mod tests {
	...
	fn test_nextcloud_config() {
		let config = NextcloudConfig::new("nextcloud", "backup/kip").with_server_url("https://cloud.example.com");
