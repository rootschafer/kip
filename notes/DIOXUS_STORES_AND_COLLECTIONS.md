...
# [Reactive Stores and Collections](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#reactive-stores-and-collections)
...
- "What exactly re-renders when I modify just _one_ entry in a collection?"
- "What if I can't wrap every value in my app in a signal?"

As our apps become more interesting, we're more likely to pull in third-party libraries and organize our state into collections like HashMaps, BTrees, and Vectors. In these cases, we need to interact with data structures _we don't own._

To make working with structs and collections easier, Dioxus provides the **Store**.

## [Reactive Stores](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#reactive-stores)
...
### [The Store<Value, Lens> type](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#the-storevalue-lens-type)
...
fn clear_title(state: Store<HeaderState, impl Writable<Target = HeaderState>>) {    state.title().take();
}
```
...
fn app() -> Element {
    ...
    rsx! {        // the lens returned by `.title()` decays into a `ReadSignal` automatically!
...
### [Stores are Readable and Writable](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#stores-are-readable-and-writable)
...
### [Nested Stores](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#nested-stores)
...
The ability for a store to "zoom in" through nested datastructures is dependent on whether or not types also implement `Store`. If, for example, or nested structs _didn't_ implement the `Store` trait, then we can't lens them:
...
### [Result, Option, and enumerated Lenses](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#result-option-and-enumerated-lenses)
...
```rust
let len = match header.subtitle().transpose() {    Some(subtitle) => subtitle.read().len(),    None => 0,
};
```
...
```rust
let len = match header.subtitle().as_ref() {    Some(subtitle) => subtitle.chars().count(),    None => 0,
};
```
...
## [Reactive Collections](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#reactive-collections)
...
rsx! {
    ul {        for (id, data) in users.read().iter() {            li { key: "{id}", "{data.name}" }
        }    }
}
```
...
fn app() -> Element {
    let mut users = use_signal(|| HashMap::<UserId, UserData>::new());
    rsx! {        // the lifetime here is is not `'static` won't pass to the ListItem component
        for (id, user) in users.read().iter() {            ListItem { key: "{id}", user }
        }    }
}
...
fn app() -> Element {
    ...
    rsx! {        for (id, user) in users.iter() {            ListItem { key: "{id}", user }
        }    }
}
...
The `Store<HashMap<K, V>>` type is a special type that implements reactivity on a per-entry basis. When we insert or remove values from the `users` store, only _one_ re-render is queued. If we edit an individual entry in the HashMap, only a single `ListItem` will re-render.
...
## [Extending Stores with Methods](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#extending-stores-with-methods)
...
## [Making the Most of Stores](https://dioxuslabs.com/learn/0.7/essentials/basics/collections#making-the-most-of-stores)

Hopefully, the examples here highlight when stores might be useful in your applications. It might be tempting to use stores _everywhere_, but in many cases, signals and structs are good enough. Most apps won't run into performance issues where precise control over the reactivity of struct fields makes a huge difference.
