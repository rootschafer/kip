...
async fn test_full_intent_lifecycle() {
    ...
    let intents = api::list_intents(app.db()).await.expect("Should list intents");
    let found = intents.iter().find(|i| i.id == intent_id);
    assert!(found.is_some(), "Created intent should be in list");
    
    api::delete_intent(app.db(), &intent_id).await.expect("Should delete intent");
...
async fn test_location_crud() {
    ...
    let locations = api::list_locations(app.db()).await.expect("Should list locations");
    let found = locations.iter().find(|l| l.id == location_id);
    assert!(found.is_some());
    
    api::remove_location(app.db(), &location_id).await.expect("Should remove location");
