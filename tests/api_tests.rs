...
async fn test_create_intent_basic() {
    ...
    assert!(intent_id.starts_with("intent:"), "Intent ID should have correct format, got: {}", intent_id);
}
...
fn test_error_display() {
    ...
    let msg = format!("{}", err);
    assert!(msg.contains("Permission denied"), "Error message should be descriptive: {}", msg);
}
...
fn test_kip_error_display() {
    ...
    let msg = format!("{}", err);
    assert!(msg.contains("Source path does not exist"), "Error should be descriptive: {}", msg);
}
