...
pub async fn copy_job(db: &DbHandle, job_id: &RecordId) -> Result<CopyResult, CopyError> {
    ...
    let result = tokio::task::spawn_blocking(move || {
        copy_and_hash(&source, &dest, &db_clone, &job_id_clone)
    })
    .await
    ...
    match result {
        ...
        Err(err) => {
            ...
            let (new_status, error_kind) = if err.is_retryable()
                && new_attempts < job.max_attempts
            {
...
fn copy_and_hash(
    ...
    loop {
        let n = source.read(&mut buf).map_err(|e| map_io_error(e, source_path))?;
        if n == 0 {
            break;
        }

        hasher.update(&buf[..n]);
        dest.write_all(&buf[..n]).map_err(|e| map_io_error(e, dest_path))?;
