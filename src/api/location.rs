...
use crate::api::{KipError, LocationId, LocationSummary, MachineSummary, MachineKind};
...
pub async fn list_locations(db: &DbHandle) -> Result<Vec<LocationSummary>, KipError> {
    ...
    let rows: Vec<serde_json::Value> = response.take(0).map_err(|e| KipError::Database(e.to_string()))?;
...
pub async fn remove_location(db: &DbHandle, location_id: &str) -> Result<(), KipError> {
    ...
    let existing: Option<serde_json::Value> = check_response.take(0).map_err(|e| KipError::Database(e.to_string()))?;
    if existing.is_some() {
...
async fn find_location_by_path(
    db: &DbHandle,
    path: &PathBuf,
) -> Result<Option<LocationId>, KipError> {
    let mut response = db
        .db
    ...
    let row: Option<serde_json::Value> = response.take(0).map_err(|e| KipError::Database(e.to_string()))?;
...
fn expand_tilde(path: PathBuf) -> Result<PathBuf, KipError> {
    if path.starts_with("~") {
        let home = dirs::home_dir().ok_or_else(|| {
            KipError::Io(std::io::Error::new(
                std::io::ErrorKind::NotFound,
                "Home directory not found",
            ))
        })?;
        let mut result = home;
        result.push(path.strip_prefix("~").map_err(|_| {
            KipError::InvalidIntentConfig("Invalid path with ~".to_string())
        })?);
        Ok(result)
    } else {
        Ok(path)
