...
use crate::api::{
    DriveStatus, IntentCounts, KipError, ReviewCounts, StatusSummary, TransferCounts,
};
...
async fn query_intent_counts(db: &DbHandle) -> Result<IntentCounts, KipError> {
    ...
    for status in &["idle", "transferring", "complete", "needs_review"] {
        ...
        let rows: Vec<serde_json::Value> = resp.take(0).map_err(|e| KipError::Database(e.to_string()))?;
...
async fn query_review_counts(db: &DbHandle) -> Result<ReviewCounts, KipError> {
    ...
    Ok(ReviewCounts {
        total,
        by_error: std::collections::HashMap::new(),
    })
}
