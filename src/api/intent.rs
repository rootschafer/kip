...
pub async fn cancel_intent(db: &DbHandle, intent_id: &str) -> Result<(), KipError> {
    ...
    db.db
        .query("UPDATE transfer_job SET status = 'cancelled' WHERE intent = $id AND status IN ['pending', 'transferring']")
        .bind(("id", intent_id.to_string()))
        .await
        .map_err(|e| KipError::Database(e.to_string()))?
        .check()
...
async fn get_intent_status(db: &DbHandle, record_id: &surrealdb::types::RecordId) -> Result<crate::api::IntentStatus, KipError> {
    let mut response = db
        .db
    ...
    let row: Option<serde_json::Value> = response.take(0).map_err(|e| KipError::Database(e.to_string()))?;
    let status_str = row.and_then(|r| r["status"].as_str().map(|s| s.to_string())).unwrap_or_else(|| "idle".to_string());
