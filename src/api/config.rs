...
pub async fn import_backup_tool_config(
    ...
    for drive in &drives {
        if let Some(mount) = &drive.mount_point {
            ...
            if mount_path.exists() {
                match crate::api::location::add_location(
                    db,
                    mount_path,
                    Some(drive.name.clone()),
                    None,
                )
                .await
                {
    ...
    if apps_dir.exists() {
        for entry in std::fs::read_dir(&apps_dir).map_err(|e| {
            KipError::ConfigImport(format!("Failed to read apps directory: {}", e))
        })? {
            ...
            for folder in &app_config.folder_configs {
                ...
                let source_id =
                    match crate::api::location::add_location(db, source_path.clone(), None, None)
                        .await
                    {
                        Ok(id) => {
                            result.locations_created += 1;
...
fn load_drives_config(path: &PathBuf) -> Result<Vec<DriveConfig>, KipError> {
    let content = std::fs::read_to_string(path).map_err(|e| {
        KipError::ConfigImport(format!("Failed to read drives config: {}", e))
    })?;
        ...
        if let Some(array) = drives_val.as_array() {
            array
                ...
                .filter_map(|v| {
                    ...
                    let mount_point = table.get("mount_point").and_then(|v| v.as_str()).map(|s| s.to_string());
                    let max_file_size = table.get("max_file_size").and_then(|v| v.as_integer()).map(|i| i as u64);
                    Some(DriveConfig {
                        name,
                        mount_point,
                        max_file_size,
...
fn load_app_config(path: &PathBuf) -> Result<AppConfig, KipError> {
    ...
    let config: AppConfig = toml::from_str(&content)
        .map_err(|e| KipError::ConfigImport(format!("Failed to parse app config: {}", e)))?;
