...
use actix_dioxus_serve::{include_dioxus_html, serve_dioxus_assets, serve_dioxus_html, DioxusAssetsConfig};
...
async fn main() -> std::io::Result<()> {
	...
	HttpServer::new(move || App::new().service(index).service(assets))
		.bind(("127.0.0.1", 8080))?
		.run()
		.await
}
