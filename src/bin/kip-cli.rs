...
async fn cmd_status(db: &db::DbHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if status.drives.connected.is_empty() {
        ...
        for drive in &status.drives.connected {
            ...
            println!("    ✅ {}    {}    {} available", drive.name, drive.mount_point.as_deref().unwrap_or("N/A"), capacity);
        }
    }
    ...
    match command {
        ConfigCommands::Import {
            ...
            if dry_run {
                ...
                println!("  Config directory: {}", config_dir.as_ref().unwrap_or(&dirs::config_dir().unwrap().join("backup-tool")).display());
                println!("\n  Would scan for:");
                println!("    - drives.toml");
                println!("    - apps/*.toml");
...
fn parse_resolution(option: &str) -> Result<api::Resolution, Box<dyn std::error::Error>> {
    Ok(match option.to_lowercase().as_str() {
        ...
        _ => return Err(format!("Unknown resolution: {}. Use: retry, skip, overwrite, delete-source, delete-dest, or abort", option).into()),
    })
}
