...
use daemon::{load_graph_data, DbHandle, Graph};
use kip_core::{ContainerView, Vec2};

use crate::ui::{
	container_components::*, file_picker::*, graph_context_menu::*, graph_edges::*, graph_nodes::*,
			...
			div {
				...
				onmousemove: move |e: MouseEvent| {
				    ...
				    match &drag_state_snapshot {
				        daemon::DragState::CreatingEdge { source_id, source_x, source_y, .. } => {
				            graph
				                .with_mut(|g| {
				        ...
				        daemon::DragState::ClickPending { node_id, start_x, start_y, .. } => {
				        ...
				        daemon::DragState::Dragging { node_id, offset_x, offset_y } => {
				...
				onmouseup: {
				    ...
				    move |_| {
				        ...
				        match current_drag {
				            ...
				            // Save position to DB
				            daemon::DragState::Lasso { start_x, start_y, current_x, current_y } => {
				                        ...
				                        if kind.is_expandable() {
				                            ...
				                            if is_machine_or_drive && path.is_empty() {
				                                let mount_point = graph_signal()
				                                    .containers
				                                    .iter()
				                                    .find(|c| { daemon::rid_string(&c.id) == node_id })
								...
								button {
									...
									onclick: {
									    ...
									    move |_| {
									        ...
									        spawn(async move {
									            match daemon::add_remote_machine(&db, &name, &host, &user).await {
									                Ok(()) => {
